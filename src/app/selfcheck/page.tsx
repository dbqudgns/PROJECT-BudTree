...
//     try {
//       // 4. API 호출 (POST 요청)
//       const response = await apiClient.post("/survey/save", requestBody);
...
export default function Page() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const results = questions.map((question, index) => ({ question, score: answers[index] }));
      const totalScore = results.reduce((sum, r) => (r.score >= 0 ? sum + r.score : sum), 0);

      sessionStorage.setItem("selfcheckResults", JSON.stringify(results));
