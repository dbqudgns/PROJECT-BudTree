...
<details> <summary><strong>데이터 전체 조회 시 응답 속도 저하 현상 </strong></summary>
     ...
     서비스에서 사용자의 Post, Survey, Chatroom, Message 데이터를 전체 조회 시 데이터 양이 많아질수록 서버 응답 속도가 점차 느려지는 문제를 확인하였고 
     수십만 건 이상 데이터를 조회하는 경우 일시적인 프리징 현상도 나타나는 것을 확인하였다. 
     이러한 현상은 사용자 경험에 부정적인 현상을 주며 서비스 확장성 측면에서도 위험 요인이 될 수 있다고 판단하였다. 
  
     - 해결 방안 : 
     
     1. 커서 기반 페이지네이션 도입
     Spring Data에서 제공하는 Slice 객체를 활용해 커서 기반 페이지네이션를 구현하였다. 
     Slice는 내부적으로 size + 1만큼 데이터를 조회하여 실제로는 요청한 개수보다 1건 더 가져온 뒤 hasNext() 여부를 판단할 수 있다. 
     위 방식을 기반으로 Repository를 구성하고 조회 결과에서 마지막 요소의 Id를 cursorId로 반환하도록 하여 다음 페이지 요청 시 기준 포인터로 활용할 수 있도록 설계하였다. 
