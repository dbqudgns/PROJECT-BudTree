...
<details> <summary><strong>데이터 전체 조회 시 응답 속도 저하 현상 </strong></summary>
     ...
     특히, 약 500,000건의 데이터를 조회하는 시나리오로 부하 테스트를 진행한 결과 일시적인 프리징 현상도 나타나는 것을 테스트하였다. 
     ...
     커서 기반 페이지네이션을 도메인마다 개별 구현하면서 반복되던 로직을 개선하기 위해 QueryDSL 기반의 공통 모듈 Repository를 설계하였다. 
     ...
     또한, 년/월 조건 필터를 WHERE절에 다중 파라미터 방식으로 적용하여 추후 다른 메서드에서도 호출 시 재사용 가능하도록 설계하였다.
